...
<form action="" name="update" method="post">
	<ul id="ul_gal" class="col-md-9">
		...
		<?php if (is_file(UPLOAD_PATH.$value['image'])): ?>
			<li>
				<a class="zoom" rel="group" href="<?php echo UPLOADS.$value['image'] ?>" title="<?php echo $value['caption'] ?>">
					<img src="<?php echo UPLOADS.$value['image'] ?>" alt="<?php  echo $value['caption'] ?>" <?php echo is_file(UPLOAD_PATH_THUMB.$value['image']) ? getSize(4, UPLOAD_PATH_THUMB.$value['image']) : getSize(4, UPLOAD_PATH.$value['image']); ?> />
				</a>
